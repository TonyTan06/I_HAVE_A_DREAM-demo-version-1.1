...
// GameWorld 提交发射参数与本帧可受击目标，不直接遍历或删除子弹。
...
public:
    struct SpawnInfo {
        ...
        float damage; // 旧发射参数，保留供上层迁移；当前实体伤害固定为一次受击
    ...
    struct Impact {
        ...
        float textX; // GameWorld 转交给 Scene 的伤害数字 x 坐标
        float textY; // GameWorld 转交给 Scene 的伤害数字 y 坐标
        bool blocked; // true 表示弹道被防御机制拦截而非造成伤害
    };

    void spawn(const SpawnInfo& spawnInfo); // 按给定参数创建并保存一枚新弹道
    void clear(); // prototype 复活时删除当前生命留下的全部弹道
