...
Rectangle CombatSystem::makeMeleeAttackHitbox(
    ...
    const float attackX = facingRight ? attacker.getX() + attacker.getHitboxWidth() : attacker.getX() - attackRange_;
    return Rectangle{
        attackX,
...
Rectangle CombatSystem::makePlayerDefenseHitbox(
    ...
    const float defenseX = player.isFacingRight() ? player.getX() + player.getHitboxWidth() : player.getX() - defenseRange_;
    return Rectangle{
        defenseX,
